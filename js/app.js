...
let route = 'home';
let flight = null;
let questionDeck = shuffledQuestions();
...
function nextQuestion() {
  if (!questionDeck.length) questionDeck = shuffledQuestions();
...
function showQuiz(meta) {
  ...
  document.getElementById('quiz-category').textContent = question.icon + ' ' + question.category + ' · Portal ' + meta.number;
...
function answerQuestion(selectedIndex) {
  ...
  if (selectedIndex === currentQuestion.answer) {
    ...
    quizTimer = window.setTimeout(() => {
      ...
      showToast('⛽ +38% COMBUSTIBLE', 'success');
...
settingsDialog.addEventListener('close', () => flight?.resume());
